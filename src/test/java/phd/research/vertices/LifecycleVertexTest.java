...
public class LifecycleVertexTest {
    ...
    private final String LABEL = String.format("Lifecycle{method=<ActivityA: %s %s(View)>", RETURN_TYPE, METHOD_NAME);
    private final String SIGNATURE = String.format("<%s: %s %s(%s)>", CLASS_NAME, RETURN_TYPE, METHOD_NAME, PARAM_TYPE);

    private LifecycleVertex v;
    ...
    public void setUp() {
        ...
        this.v = new LifecycleVertex(method);
    }

    @Test
    public void testConstructor() {
        assertEquals("Type should be 'lifecycle'.", Type.lifecycle, this.v.getType());
    ...
    public void testToString() {
        assertEquals("Wrong string value returned.",
                String.format("Lifecycle{label='%s', visit=false, localVisit=false, method=%s}", LABEL, SIGNATURE),
                this.v.toString()
                    );
    }
}