...
public class CallbackVertexTest {
    ...
    private final String LABEL = String.format("Callback{method=<ActivityA: %s %s(View)>", RETURN_TYPE, METHOD_NAME);
    private final String SIGNATURE = String.format("<%s: %s %s(%s)>", CLASS_NAME, RETURN_TYPE, METHOD_NAME, PARAM_TYPE);

    private CallbackVertex v;
    ...
    public void setUp() {
        ...
        this.v = new CallbackVertex(method);
    }
    ...
    public void getAttributes() {
        ...
        assertEquals("Wrong type attribute returned.", "callback", attributes.get("type").getValue());
        assertEquals("Wrong label attribute returned.", LABEL, attributes.get("label").getValue());
        assertEquals("Wrong color attribute returned.", "purple", attributes.get("color").getValue());
        assertEquals("Wrong shape attribute returned.", "hexagon", attributes.get("shape").getValue());
    ...
    public void testToString() {
        assertEquals("Wrong string value returned.",
                String.format("Callback{label='%s', visit=false, localVisit=false, method=%s}", LABEL, SIGNATURE),
                this.v.toString()
                    );
    }
}
